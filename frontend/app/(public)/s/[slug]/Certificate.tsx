...
// Formatea la fecha (ISO o similar) en el idioma activo; si no parsea, la
// devuelve tal cual.
...
export function Certificate({
  ...
  // `t` cambia de identidad al cambiar de idioma; lo leemos por ref para no
  // reejecutar el efecto (y reemitir el certificado) al cambiar de idioma.
  const tRef = useRef(t);
  tRef.current = t;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (cancelled) return;
        if (!res.ok) {
          setError((body && body.detail) || tRef.current("public.cert.error.issue"));
          return;
        }
        setData(body as CertificateData);
      } catch {
        if (!cancelled) setError(tRef.current("public.access.error.connect"));
      } finally {
    ...
    <div className="enc-cert-overlay fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto bg-neutral-900/60 p-4 sm:p-8">
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="enc-print-hide mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          >
            <X className="h-4 w-4" /> {t("public.cert.close")}
          </button>
          {data && (
            <button
              type="button"
            ...
            >
              <Printer className="h-4 w-4" /> {t("public.cert.print")}
            </button>
          )}
        </div>
        ...
        {loading && (
          <div className="flex items-center justify-center gap-2 rounded-2xl bg-white px-6 py-16 text-sm text-neutral-500 shadow-sm">
            <Loader2 className="h-5 w-5 animate-spin" /> {t("public.cert.issuing")}
          </div>
        )}
          ...
          >
            ...
            <div className="relative flex flex-col items-center text-center">
              ...
              <p className="mt-1 text-sm text-neutral-500">
                {t("public.cert.subtitle")}
              </p>

              <p className="mt-8 text-sm text-neutral-500">
                {t("public.cert.certifies")}
              </p>
              ...
              <p className="mt-6 max-w-lg text-base leading-relaxed text-neutral-700">
                {t("public.cert.passedPre")}
                <span className="font-semibold">{data.survey_title}</span>
                {t("public.cert.passedMid")}
                <span className="font-bold" style={{ color: accent }}>
                  {data.percent}%
                </span>
                {t("public.cert.passedPost")}
              </p>

              <p className="mt-2 text-xs text-neutral-400">
                {t("public.cert.passingScore", { score: data.passing_score })}
              </p>
